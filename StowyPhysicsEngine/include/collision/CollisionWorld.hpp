...
public:
     ...
     */
    void SolveCollisions(std::vector<Collision>& collisions, float deltaTime) const;
    ...
    void WarmStaring(std::vector<Collision>& collisions, float deltaTime);

protected:
    std::unordered_map<std::uint64_t, CollisionBody*> _bodies;
    std::vector<Solver*> _solvers;
    std::vector<Collision> old_collisions;
