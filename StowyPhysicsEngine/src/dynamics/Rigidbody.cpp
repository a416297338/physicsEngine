...
{
Rigidbody::Rigidbody()
	: _invMass(1), _angular_velocity(0),
	_takesGravity(true)
{
	_isDynamic = true;
}
...
{
	return _invMass * _collider->GetInvInertia();
}
...
Vector2 Rigidbody::GetCenter()
{
	return _transform.position + _collider->center;
}
void Rigidbody::UpdateImpulse(const Vector2& impulse,const float& angular_impulse)
{
	_velocity += impulse * InvMass();
	_angular_velocity += InvInertia() * angular_impulse;
}
